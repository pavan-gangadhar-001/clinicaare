...
class DoctorAddAppointmentStep1Screen extends StatefulWidget {
  ...
  @override
  State<DoctorAddAppointmentStep1Screen> createState() =>
      _DoctorAddAppointmentStep1ScreenState();
}

class _DoctorAddAppointmentStep1ScreenState
    extends State<DoctorAddAppointmentStep1Screen> {
  ...
  void init() async {
    ...
    if (isUpdate) {
      ...
      if (widget.appointmentData!.visit_type.validate().isNotEmpty) {
        multiSelectStore.selectedService.addAll(
            widget.appointmentData!.visit_type!.map((e) => ServiceData(
                id: e.id, name: e.service_name, service_id: e.service_id)));
        servicesCont.text = "${multiSelectStore.selectedService.length} " +
            locale.lblServicesSelected;
        appointmentAppStore.addSelectedService(multiSelectStore.selectedService
            .map((element) => element.id.toInt())
            .toList());
        appointmentAppStore.addReportListString(
            data: widget.appointmentData!.appointment_report.validate());
      }
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget(locale.lblStep1,
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          ...
          //   formKey.currentState!.save();
            ...
            DoctorAddAppointmentStep2Screen(
